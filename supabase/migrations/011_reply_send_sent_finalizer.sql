-- HireX Sales Agent - Phase 2B.4F-A transactional REPLY SENT finalizer.
-- This migration only finalizes an already accepted REPLY and does not alter NEW-send functions.

begin;

create function public.finalize_reply_email_send_request_sent(
  p_request_id uuid,
  p_workspace_id uuid,
...
begin
  if p_request_id is null
    ...
    or p_provider_message_id !~ '^[A-Za-z0-9_-]{1,512}$'
    or p_provider_thread_id !~ '^[A-Za-z0-9_-]{1,512}$' then
    return false;
  end if;
