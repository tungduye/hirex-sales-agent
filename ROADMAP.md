...
- Phase 2B.2 explicit Gmail send consent is complete. Phase 2B.3A passed one controlled NEW plain-text live send and History reconciliation by provider message/thread IDs. Gmail replaced the requested Message-ID. Phase 2B.3B prepares internal persistence for the unverified `X-HireX-Send-Request-ID` candidate without FK/uniqueness, reconciliation, unlock, or retry; no second header-preservation live test has occurred. Phase 2B.4 adds reply/thread sending derived from trusted mailbox state.
