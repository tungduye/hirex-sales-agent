...
- Phase 2B.2 explicit Gmail send consent is complete. Phase 2B.3A passed a controlled NEW plain-text send; Gmail replaced the requested Message-ID. Phase 2B.3B persists internal `X-HireX-Send-Request-ID` metadata without FK/uniqueness. Phase 2B.3C verified preservation in one controlled send plus History sync, but the header remains untrusted and requires future multi-signal reconciliation; no reconciliation, unlock, or retry exists. Phase 2B.4 adds reply/thread sending derived from trusted mailbox state.
