...
export async function connectFacebookPage(_state:ChannelActionState,formData:FormData):Promise<ChannelActionState>{
  ...
  try{const client=createPrivilegedClient();const {data,error}=await client.rpc("upsert_channel_account_credential",{p_workspace_id:account.workspaceId,p_connected_by:account.userId,p_channel_type:"FACEBOOK",p_provider:"META_GRAPH",p_external_account_id:parsed.data.pageId,p_display_name:parsed.data.displayName||null,p_capabilities:["SEND_TEXT","REPLY","DELIVERY_RECEIPTS","READ_RECEIPTS"],p_access_token_encrypted:encryptChannelCredential(parsed.data.pageAccessToken),p_webhook_secret_encrypted:encryptChannelCredential(parsed.data.appSecret),p_configuration:{graphApiVersion:parsed.data.graphApiVersion}});if(error||typeof data!=="string")return{status:"error",message:"Facebook Page could not be connected."};revalidatePath("/settings/channels");return{status:"success",message:"Facebook Page credential saved securely."}}catch{return{status:"error",message:"Facebook Page connection is unavailable."}}
}
