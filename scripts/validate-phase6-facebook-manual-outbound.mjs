...
const dependencies = {adapters,createLockId:()=>"lock-1",now:()=>"2026-09-14T00:00:00.000Z",loadAction:async()=>status==="APPROVED"?action:null,loadPolicy:async()=>({id:action.policyDecisionId,workspaceId,allowed:true,expiresAt:"2026-09-14T00:01:00.000Z"}),isSuppressed:async()=>false,claim:async()=>{claimCalls += 1; if(status!=="APPROVED")return null;status="EXECUTING";return {...action,status:"EXECUTING",executionLockId:"lock-1"}},loadClaimedAttachments:async()=>[],finalizeSent:async()=>{sentFinalizations += 1;status="SENT";return true},finalizeFailed:async()=>false,finalizeUnknown:async()=>false};
