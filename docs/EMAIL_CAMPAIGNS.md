...
Sequence campaigns support one initial email and up to five follow-ups. Phase 4AB single-send campaigns remain on their original path. See `PHASE_4C_ARCHITECTURE.md` for lifecycle, threading, stop-signal, quota and export guarantees. The Phase 4C migration and live tests are intentionally not executed by implementation work.
