...
The operator output is a minimal safe report containing local IDs, lifecycle classifications, an allow-listed `executorReason`, and safe provider IDs only. Reviewed pre-claim reasons such as `CREDENTIALS_UNAVAILABLE`, `LOCK_GENERATION_FAILED`, and `CLAIM_UNAVAILABLE` remain distinguishable without exposing exceptions. Unknown or malformed reasons fail closed to `EXECUTION_UNAVAILABLE`. The report never emits body, recipient, subject, parent RFC Message-ID, execution lock, MIME, credentials, tokens, raw rows, provider responses, or database errors. This implementation has no route, UI, scheduler, batch, campaign, or automation caller, and its real live invocation remains a separate manual review action.
