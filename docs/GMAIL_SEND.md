...
The operator output is a minimal safe report containing local IDs, lifecycle classifications, and safe provider IDs only. It never emits body, recipient, subject, parent RFC Message-ID, execution lock, MIME, credentials, tokens, raw rows, provider responses, or database errors. This implementation has no route, UI, scheduler, batch, campaign, or automation caller, and its real live invocation remains a separate manual review action.
