...
The authenticated Sales Inbox now provides a bounded conversation list, chronological plain-text thread history, canonical latest-safe-inbound target resolution, an editable AI draft, and an explicit human-approved Send action. The AI adapter receives only a bounded recent message projection and has no Gmail tools; email content is treated as untrusted data. Sending delegates to the existing idempotent REPLY creation and exact-one executor boundaries. `DELIVERY_STATUS_UNKNOWN` locks immediate retry and never generates a replacement idempotency key. Mailbox synchronization remains authoritative for the sent conversation copy.

Manual product test plan (not executed during implementation): open HireX Inbox, select the controlled test conversation, generate an AI draft, edit it, click Send once, verify `SENT`, verify the controlled recipient receives it in the same Gmail thread, then verify the conversation refreshes after mailbox sync.

## Phase 2B.1 through 2B.4G-FINAL status

Phase 2B.1 schema is complete. Phase 2B.2 send consent is complete and live-tested. Phase 2B.3A one-message send passed its controlled live test. Phase 2B.3B persists Gmail-observed `X-HireX-Send-Request-ID` metadata. Phase 2B.3C records one successful controlled correlation-header preservation test. Phase 2B.3D adds a server-only, read-only ambiguous-send evidence evaluator. Phase 2B.3E adds the database-side transactional reconciliation finalizer. Phase 2B.3F adds the exact one-request orchestrator. Phase 2B.3G adds bounded read-only discovery. Phase 2B.3H adds the manual exact-request boundary. Phase 2B.3I-A adds local read-only list/inspect commands, and Phase 2B.3I-B adds an explicit local operator reconcile command. Phase 2B.4A adds read-only validation of one exact canonical Gmail message as a future reply target. Phase 2B.4B adds server-only creation of one idempotent `PENDING` reply intent. Phase 2B.4C adds a separate service-role-only transactional first-attempt claim for that reply intent. Phase 2B.4D adds read-only post-claim canonical execution planning. Phase 2B.4E adds a pure plain-text Gmail reply MIME builder. Phase 2B.4F-A adds a REPLY-specific transactional SENT finalizer. Phase 2B.4F-B only extends the reviewed Gmail send transport with an optional provider thread ID. Phase 2B.4F-C1 adds a separate deterministic REPLY FAILED finalizer and intentionally does not add a catch-all failure code. Phase 2B.4F-C adds a server-only, exact-one-request executor foundation with no production caller. Phase 2B.4G-A adds a guarded operator-only read-only preflight for one explicitly selected inbound message. Phase 2B.4G-B adds a separately guarded operator-only command that may create exactly one immutable `PENDING` REPLY request through the reviewed Phase 2B.4B boundary. Phase 2B.4G-FINAL consolidates preflight, explicit confirmation, idempotent creation, exact pending inspection, one executor invocation, and exact final inspection behind a separate manual operator guard. Automatic reconciliation, retries, HTML, attachments, multiple recipients, workers, scheduled follow-ups, campaigns, and AI-triggered delivery remain inactive.
...
### Phase 2B.4G-B controlled reply-request creation

The operator command `scripts/create-controlled-gmail-reply-request.mjs` accepts exactly one explicit workspace UUID, Gmail account UUID, canonical local inbound message UUID, plain-text reply body, and UUID idempotency key. It has no defaults, target discovery, latest-message selection, or interactive fallback and refuses to run unless `HIREX_ENABLE_CONTROLLED_REPLY_REQUEST_CREATE=1` exactly. The guard authorizes only creation of a durable reply intent; it is not claim or Gmail-send authorization.

The command delegates once to the existing server-only `createReplySendRequest()` boundary and performs no direct database mutation. That boundary re-evaluates the canonical reply target and derives recipient and subject server-side. A successful result is exactly one `REPLY` request in `PENDING` with `attempt_count=0`; a repeated compatible idempotency key returns the existing compatible `PENDING` request, while incompatible reuse fails closed without overwrite or reset.

The safe operator result contains only the reviewed local IDs, status/reason, `requestStatus=PENDING`, and `sendType=REPLY`. It exposes no addresses, subject, body, provider/thread/RFC identifiers, credentials, tokens, raw errors, or database details. Phase 2B.4G-B does not claim, finalize, execute, call Gmail, retry, batch, schedule, or add a route/UI. Before any controlled delivery, a separate future phase must inspect the exact created request and explicitly authorize execution of that request once.

### Phase 2B.4G-FINAL consolidated controlled live verification

The manual operator command `scripts/run-controlled-gmail-reply-live-test.mjs` performs one fixed flow: **PREFLIGHT → CREATE → INSPECT PENDING → EXECUTE ONCE → INSPECT FINAL**. It requires all target, body, idempotency, expected canonical evidence, and literal confirmation flags explicitly, plus `HIREX_ENABLE_CONTROLLED_REPLY_LIVE_TEST=1`. Expected recipient, subject, provider thread, and parent RFC Message-ID are equality assertions only; canonical delivery evidence still comes exclusively from reviewed server boundaries.

The exact pending inspection re-reads the created request using request/workspace/account scope and verifies the pristine `REPLY/PENDING/attempt_count=0` lifecycle, immutable target/content/idempotency, canonical single recipient and subject, and absence of locks, scheduling, HTML, provider evidence, RFC output ID, and sent timestamp. Any mismatch stops before execution without modifying the request.

The operator invokes `sendOneReplyMessage()` **at most once** and never calls Gmail transport directly. It then inspects the same request once. An exact terminal state becomes `VERIFIED_SENT` or `VERIFIED_FAILED`; ambiguous delivery or inconsistent SENT verification remains `DELIVERY_STATUS_UNKNOWN` or unavailable and is never repaired by resend. There is no automatic retry for `SENDING`, `DELIVERY_STATUS_UNKNOWN`, provider-thread mismatch, finalizer uncertainty, or final verification mismatch. A crash/rerun uses the same caller-supplied idempotency key: only a still-pristine compatible `PENDING` request may continue; an attempted, `SENDING`, `SENT`, `FAILED`, or otherwise incompatible row conflicts and is never reset.

Before orchestration can create a request, the operator resolves and validates the complete production module graph, including canonical evaluation, creation, inspection, executor, claim, planner, credentials, Gmail transport, and both finalizers. A bootstrap/import failure therefore returns `LIVE_TEST_BOOTSTRAP_UNAVAILABLE` at `failureStage=BOOTSTRAP` before any mutation. This specifically avoids loading the executor for the first time only after a durable `PENDING` request has been created.

The operator output is a minimal safe report containing local IDs, lifecycle classifications, a closed `failureStage`, an allow-listed `executorReason`, and safe provider IDs only. Reviewed pre-claim reasons such as `CREDENTIALS_UNAVAILABLE`, `LOCK_GENERATION_FAILED`, and `CLAIM_UNAVAILABLE` remain distinguishable without exposing exceptions. A thrown executor invocation is distinct as `EXECUTOR_INVOCATION_UNAVAILABLE`; malformed executor output is `EXECUTOR_RESULT_UNAVAILABLE`. The report never emits body, recipient, subject, parent RFC Message-ID, execution lock, MIME, credentials, tokens, raw rows, provider responses, or database errors. This implementation has no route, UI, scheduler, batch, campaign, or automation caller, and its real live invocation remains a separate manual review action.
